...
def main(args):
    ...
    real_death=[]
    real_death_all=[]
    with open('./mortality_timeseries_prov.csv','r') as csvfile:
#        reader = csv.reader(csvfile)
        ...
        real_date=datetime.date(int(row['date_death_report'][-4:]),int(row['date_death_report'][3:5]),int(row['date_death_report'][:2]))
...
#    print(params_dict)
    ...
    for params_dict['INITIAL_R_0'] in np.linspace(IR*0.7,IR):
        for params_dict['LOCKDOWN_R_0'] in np.linspace(LR*0.95,LR*1.05,20):
            for params_dict['DAILY_IMPORTS'] in np.linspace(DI,DI*1.35):
                for params_dict['MORTALITY_RATE'] in np.linspace(MR*1.5,MR*2.5,20):
                    params_tups = tuple(params_dict.items())
                    region_model.init_params(params_tups)
                    dates, infections, hospitalizations, deaths = run(region_model)
                    deaths_total = deaths.cumsum()
                    ...
                    if error <= best_error:
                        best_error = error
                        inf_proj, hosp_proj, death_proj=infections, hospitalizations, deaths
                        best_parameters = {'INITIAL_R_0':params_dict['INITIAL_R_0'],
    ...
    plt.legend(loc='best')
    plt.savefig(f"Proj_{args.country}_{args.subregion}.png", dpi=1200)
    plt.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--best_params_dir', default='best_params/latest',
    ...
    parser.add_argument('--region', default='',
        help='only necessary if loading params from --best_params_dir')
    parser.add_argument('--subregion', default='Ontario',
        help='only necessary if loading params from --best_params_dir')
